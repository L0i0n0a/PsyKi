...
export default function AccuracyComparison() {
  ...
  const scaleThickness = (percent: number) =>
    minThickness + ((percent / 100) * (maxThickness - minThickness));

  const menschThickness = scaleThickness(menschPercent);
  const kiThickness = scaleThickness(kiPercent);
  
  function generateGrowingThicknessLines(
  p0: [number, number],
  p1: [number, number],
  p2: [number, number],
  color: string,
        ...
        <div className="flex items-center justify-between relative">
          ...
          <div className="relative flex-1 mx-4">
            <div
              ...
              style={{
                background:
                  "linear-gradient(to right, #FB8C00 , #FFE0B2 50%,  #90CAF9 50%, #0D47A1)",
              }}
            >
              {/* Z-Score Bubble */}
